...
from .sort import *
from .it_sort import it_process_multiple
...
from .process_complex import *
from .process_complex_cython import read_complexes_cython
# Add the following import at the top of the file for mem profiling
'''try:
...
# this is the version that tries to use the index scheme
# it currently works for the repeat regions files but it is slower and more memory intensive than original
# it is possible that the gains are not pronounced since it is just repeated lines and a chia-pet instead of 
# larger complex file and multiple regions
...
def start(path1, path2, processing_type, graphs,
    ...
    if processing_type == "abc":
        ...
        # gets start of a to end of c region
        ...
        # use either the cython or the python version
        #chroms, ids = read_complexes(path1)
        chroms, ids = read_complexes_cython(path1)


        seen_ids = set()

        # regions should be able to fit in memory so save lines to a list to reduce file i/o
        with open(filter_regions_filename, 'r') as f:
            lines = [line.strip() for line in f if not line.startswith("#") and line.strip()]
        #with open(filter_regions_filename, 'r') as f:
        for line in lines:

                ChIA_Drop = reduce_search_space(line, chroms, ids)

                filtered_intersections = filter_intersections(ChIA_Drop, filter_regions)
                ...
                for key, ChIA_Drop_anchor in filtered_intersections.items():
                    ...
                    if id in seen_ids:
                        continue  # Skip processing if the id has already been seen
    ...
    # END ABC----------------------------------------------------------------------------------------
        ...
        filter_regions_filename = "temp_filter_regions.bed"
        generate_filter_regions(path2, filter_regions_filename)
        filter_regions = BedTool(filter_regions_filename)
        ...
        for key, ChIA_Drop_anchor in filtered_intersections.items():
            ...
            ## CHANGING TO INTERVAL TREE FOR TEST
            ranked_gems = it_process_multiple(ChIA_Drop_anchor, num_fragments_min, num_fragments_max, yes_chroms, no_chroms)
            if graph_flag == "yes":
